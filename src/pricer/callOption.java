...
public class callOption {
	...
	callOption(double sPrice, double volat, double strike, double maturity, int step, double cRate, int type){
	    ...
	    for (int i = 0; i < num_of_node; i++){
	        ...
	        tem.optionPrice = Math.max((tem.stockPrice - strike), 0);
	        tem.left = null;
	        tem.right = null;
	        myque.add(tem);
	    ...
	    while(myque.size() > 1){
	        ...
	        if (n1.step == n2.step){
	            ...
	            newnode.optionPrice = Math.max((np * n1.optionPrice + nq * n2.optionPrice) / a, newnode.stockPrice - strike);
	            myque.add(newnode);
	        }
	    }
	    head = new Node();
	...
	public double getDeltaHelper(Node n){
		if (n.left != null && n.right != null){
	        return (n.left.optionPrice - n.right.optionPrice) / (n.left.stockPrice - n.right.stockPrice);
		}
	    else
	        return 0;
	}
	...
	public double GetVega(){
		callOption temp = new callOption(stockPrice, (myVol + 0.01), myStrike, myMaturity, myStep, mycRate, myType);
	...
	public static void main(String[] args) {
		callOption c1 = new callOption(100, 0.2, 110, 1, 26, 0.05, 1);
